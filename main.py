...
# === ГЛОБАЛЬНЫЙ ЛОГГЕР ===
def log_error(message: str, exc: Exception = None, image_path: str = None, n_colors=None, blur=None, mode=None):
    log_dir = "logs"
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        ...
        if n_colors is not None:
            f.write(f"Цветов: {n_colors}\n")
        if blur is not None:
            f.write(f"Размытие: {blur}\n")
        if mode is not None:
            ...
            f.write(f"Эффект: {mode} ({mode_names.get(mode, 'Неизвестно')})\n")
        f.write(f"\nСообщение:\n{message}\n")
        if exc:
            f.write(f"\nТип исключения: {type(exc).__name__}\n")
            f.write(f"Аргументы: {exc.args}\n")
            f.write(f"\nTraceback:\n")
            f.write(traceback.format_exc())
    
    print(f"{RED}\n[ОШИБКА] Подробности: {log_path}{RESET}")
    return log_path

# === Утилиты ===
def resolve_shortcut(path):
    """Возвращает реальный путь, если это ярлык .lnk"""
    if not path.lower().endswith(".lnk"):
        return path  # ← не ярлык → возвращаем как есть
    ...
    try:
        ...
        if target and os.path.exists(target):
            ...
            print(f"{YELLOW}Ярлык указывает на несуществующий файл: {path}{RESET}")
            return path  # fallback
    except Exception as e:
        print(f"{RED}[!] Ошибка при чтении ярлыка: {e}{RESET}")
        return path  # fallback
...
def select_images_via_dialog(multi=False):
    try:
        ...
        log_error("Ошибка при открытии диалога выбора файла", e)
        return []
    finally:
        try:
            root.destroy()
    ...
    try:
        ...
        if scale != 1.0:
            ...
            if new_w <= 0 or new_h <= 0:
                raise ValueError(f"Масштаб {scale} даёт нулевой размер: {new_w}x{new_h}")
        ...
        if mode == 2:
            ...
            try:
                ...
                quantized = quantized.copy()  # fallback
        elif mode == 14:
        ...
        log_error("Ошибка в process_single", e, image_path, n_colors, blur_strength, mode)
        raise
...
if __name__ == "__main__":
    try:
        ...
        log_error("Критическая ошибка программы", e)
        print(f"{RED}Программа завершена с ошибкой. См. лог в папке 'logs'.{RESET}")